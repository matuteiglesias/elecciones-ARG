...
# %%
circuitos['electores'].sum()

# %%
data_circ.groupby(['eleccion_id', 'cargo_id'])['votos_cantidad'].sum()

# %%
merged_data = circuitos.merge(data_circ, left_on=['distrito_id', 'seccion_id', 'circuito_id'], right_on=['distrito_id', 'seccion_id', 'circuito_id'], how = 'right')

# %%
merged_data.groupby(['eleccion_id', 'cargo_id'])['votos_cantidad'].sum()

# %%
merged_data = merged_data.merge(prov_nams, left_on='distrito_id', right_on='distrito_id')
merged_data = merged_data.merge(seccion_region, on=['distrito_id', 'seccion_id', 'seccion_nombre'], how='left')


# %%
# info = merged_data.loc[(merged_data.cargo_tag == 'PR') & (merged_data.eleccion_tag == 'GRAL23n') & (merged_data.Error_bin.isin(['1', '6', '13', '25']))]
info = merged_data.loc[(merged_data.cargo_tag == 'PR') & (merged_data.eleccion_tag == 'GRAL23n')] # & (merged_data.Error_bin.isin(['1', '6', '13', '25']))]
info = info.loc[info.electores > 2500]

# %%


# %%
# merged_data.loc[(merged_data.cargo_tag == 'PR') & (merged_data.eleccion_tag == 'GRAL23n')].votos_cantidad.sum()

# %%
info.votos_cantidad.sum()

# %%
info.nunique()

# %%
merged_data['elector_bin'] = pd.cut(merged_data['electores'], bins=10, labels=False)


# %%
# plt.figure(figsize=(10, 6))
# sns.boxplot(x='elector_bin', y='votos_cantidad', data=merged_data)
# plt.title('Distribution of Votes by Elector Bin')
# plt.xlabel('Elector Bin')
# plt.ylabel('Votes')
# plt.show()
